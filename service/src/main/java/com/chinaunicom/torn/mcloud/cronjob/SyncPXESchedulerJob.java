...
@CronJobMetadata(type = CronJobType.SYNC_CLOUDBOOT, delay = 4 * 1000)
public class SyncPXESchedulerJob extends QuartzJobBean {

    private static final LogEntityFactory logFactory = new LogEntityFactory(SyncPXESchedulerJob.class, ServiceRole.SCHEDULER.name());
    ...
    protected void executeInternal(JobExecutionContext ctx) throws JobExecutionException {
        ...
        this.loggerService.info(SyncPXESchedulerJob.logFactory.product()
                .how(LogHow.SCHEDULE).what(String.format("schedule sync [area: %s] pxe", areaKey)).build());
