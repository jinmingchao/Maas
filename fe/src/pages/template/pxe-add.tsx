...
import {postAddPXE, putUpdatePXE} from '@/service/info';
...
const pxeAdd = ({
    ...
    return (
        <Modal width="80vw" visible={visible} onCancel={onClose} footer={null} title={id ? "更新PXE" : "添加PXE"}>
            <Form form={form} onFinish={(entity: IInfoForm) => {
                if (id) {
                    putUpdatePXE((selectedArea as any).id, id, entity).then(() => {
                        dispatch(syncAreaInfo((selectedArea as any).id));
                        notification.info({ message: '更新成功', description: '更新成功', duration: null });
                    })
                }
                else {
                    postAddPXE((selectedArea as any).id, entity).then(() => {
                        dispatch(syncAreaInfo((selectedArea as any).id));
